...
wss.on('connection', (socket) => {
  ...
  socket.on('message', (raw) => {
    ...
    // Любое слово от клиента — признак жизни. Ждать именно ответа на опрос
    // нельзя: на слабой машине браузер надолго уходит в отрисовку кадра.
    client.silent = 0;
...
/**
 * Тихо отваливающиеся соединения. Признаком жизни считается любое сообщение —
 * клиент шлёт свою позицию двадцать раз в секунду, — а ping-кадр нужен лишь
 * для того, чтобы разбудить совсем молчаливого.
 *
 * Терпим три пропущенных опроса подряд: три четверти минуты тишины.
 */
setInterval(() => {
  for (const client of clients.values()) {
    if (client.silent >= 3) {
