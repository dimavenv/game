...
/**
 * Дантово ущелье: узкая щель в западном боку Петушка, справа от тарзанки,
 * если стоять на её площадке лицом к реке. Внутрь ведёт один вход, дальний
 * конец глухой — возвращаться приходится тем же путём.
 */
export const GORGE = {
  /** Ось щели: от входа вверх по склону. */
  path: [
    [-137, 113],
    [-132, 107],
    [-127, 103.5],
  ...
  /** Полуширина дна и ширина скругления к стенам. */
  halfWidth: 2.3,
  rim: 5.0,
  /** На сколько метров дно опускается ниже склона в самой глубокой части. */
  depth: 11,
  /** Уклон дна: сколько метров подъёма на метр пути. Щель почти ровная. */
  rise: 0.14,
  /** Табличка «ДАНТОВО УЩЕЛЬЕ» у входа. */
  sign: { x: -139.5, z: 116 },
  /** Сколько дроздов может оказаться внутри. */
  minBirds: 5,
  maxBirds: 13,
  /** Награда за верный счёт. */
  reward: 350,
} as const;
...
export const ZOMBIE = {
  ...
  /**
   * Пещерные. Живут в ходах круглые сутки и наружу не выходят: стены пещеры
   * их держат так же, как игрока. Появляются, когда игрок заходит внутрь.
   */
  cave: {
    /** Сколько мертвецов сидит в одной пещере. */
    count: 3,
    /** Ближе этого ко входу не появляются: сразу в зубы никто не хочет. */
    fromMouth: 12,
    /** Когда игрок отошёл от зева дальше — пещера пустеет. */
    despawn: 110,
  },
} as const;
